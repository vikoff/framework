...
class Alias_AdminController extends Controller {
	...
	const TPL_PATH = 'modules/Alias/templates/';
	...
	public function display_list($params = array()){
		...
		$collection = new Alias_Collection();
		$variables = array(
	...
	public function display_new($params = array()){
		...
		$variables = array_merge(Tools::unescape($_POST), array(
			'instanceId' => 0,
			'pageTitle'  => $pageTitle,
			'validation' => Alias_Model::create()->getValidator()->getJsRules(),
		));
	...
	public function display_edit($params = array()){
		...
		$instance = Alias_Model::load($instanceId);
	...
	public function action_save($params = array()){
		...
		$instance = new Alias_Model($instanceId);
	...
	public function action_delete($params = array()){
		...
		$this->setRedirectUrl('admin/config/alias/list');
